...
for (name in unique(rb.data$name)) {
  ...
  for (i in 1:nrow(player)) {
    ...
    rb.coors <- rbind(
       rb.coors,
       list(
          x = sum(c(1, lag1, lag2, lag3, player.now$age) * rb.model$coefficients, na.rm = TRUE),
          y = player[i+1,"fantpt"]
       )
    )
  }
...
wr.formula <- formula(fantpt ~ lag(fantpt, 1:2) + age)
...
for (name in unique(wr.data$name)) {
  ...
  for (i in 1:nrow(player)) {
    ...
    wr.coors <- rbind(
       wr.coors,
       list(
          x = sum(c(1, lag1, lag2, player.now$age) * wr.model$coefficients, na.rm = TRUE),
          y = player[i+1,"fantpt"]
       )
    )
  }
